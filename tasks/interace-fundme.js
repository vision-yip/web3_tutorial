...
task('interact-fundme', 'interact with fundme contract').addParam('addr', 'fundme contract address').setAction(async(taskArgs, hre) => {
  ...
  const firstAccountBalanceInFundMe = await fundMe.fundersToAmount[firstAccount];
  const secondAccountBalanceInFundMe = await fundMe.fundersToAmount[secondAccount];
